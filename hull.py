...
def max_dist_line_point(pointA, pointB, points):
    # Calculate the maximum distant point from a line for initial simplex
    maxDist = 0
    maxDistPoint = None  # Initialize maxDistPoint to None or an appropriate default value
    for point in points:
...
def max_dist_plane_point(plane, points):  # Calculate the maximum distance from the plane
    maxDist = 0
    maxDistPoint = None
    for point in points:
...
def initial():  # To calculate the extreme points to make the initial simplex
    ...
    # Initialize extreme points with the first point in the list
    x_max = x_min = y_max = y_min = z_max = z_min = points[0]
...
try:
    with open(sys.argv[1], "r") as data:
        ...
        for line in data:
            a = [float(value) for value in line.split()]
...
try:
    ...
    third_point = max_dist_line_point(initial_line[0], initial_line[1],
                                      points)  # Calculate the 3rd point to make a plane
    first_plane = Plane(initial_line[0], initial_line[1], third_point)  # Make the initial plane by joining 3rd point
    # to the line

    fourth_point = max_dist_plane_point(first_plane, points)  # Make the fourth plane to make a tetrahedron
except Exception as e:
    print(f"Figure either in 2D or 3D: {e}")
    sys.exit()

possible_internal_points = [initial_line[0], initial_line[1], third_point, fourth_point]  # List that helps in
# calculating orientation of point
...
list_of_planes = [first_plane, second_plane, third_plane, fourth_plane]  # List containing all the planes
